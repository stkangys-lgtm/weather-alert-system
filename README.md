...
- **Phase 2 (진행중)**: 이상기상(강풍/호우/폭염) 판정 로직 + GitHub Pages 대시보드 + 공고문 텍스트 자동 생성. 담당자 개별 알림 발송(카카오톡/이메일/슬랙)은 추후 예정 — 현재는 공고문 텍스트를 수동으로 단톡방에 공유하는 방식

## 대시보드 (GitHub Pages)

매 실행마다 현장별 실황·예보·이상기상 판정 결과를 `docs/index.html`에 정적 페이지로 생성하고
자동 커밋합니다. **담당자 이름·연락처는 공개 페이지 특성상 포함하지 않습니다** (개인정보 보호).
담당자 연락처가 포함된 공고문 텍스트는 `docs/announcement.txt`에 별도로 저장되며, 이 파일은
저장소에 접근 권한이 있는 사람만 볼 수 있습니다 (Pages로 게시되지 않음).

이상기상 판정 기준(산업안전보건기준에 관한 규칙 제37조 및 온열질환 예방 가이드 참고,
`src/alert_rules.py`에서 조정 가능):

| 항목 | 주의 | 경보 |
|---|---|---|
| 강풍 (풍속) | 10m/s 이상 | 15m/s 이상 |
| 호우 (1시간 강수량) | 1mm 이상 | 15mm 이상 |
| 폭염 (기온) | 33°C 이상 | 35°C 이상 |

> 법령상 강풍 기준은 순간풍속이나, 기상청 API는 1시간 평균풍속만 제공하므로 이를 근사치로 사용합니다.
...
│   └── main.py            # 실행 진입점
├── docs/                 # GitHub Pages 대시보드 (index.html) + 공고문(announcement.txt)
